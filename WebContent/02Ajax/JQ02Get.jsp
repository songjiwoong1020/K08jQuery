...
$.get()
	...
	사용법
		...
		-url : 정보를 요청할 경로
		-param : 서버로 전송 할 파라미터로 json형태로 기술
		-success(data) : 요청이 성공 했을 때 실행되는 콜백(callback)메소드
			콜백메소드가 호출 될 때 전달되는 파라미터를 통해 성공 혹은
			실패여부를 판단 할 수 있다.
	※ 서버로 요청시 전송할 값(파라미터)이 없는 경우에는 생략 가능함.
...
$(function(){
    ...
    파라미터를 전송 할 때는 항상 JSON으로 조립해야 한다.
    하지만 폼값이 많아서 조립이 힌든경우에는 serialize()를 사용한다.
